...
public class CateUpdateServlet extends HttpServlet {
	...
	CategoryService caservice=new CategoryServiceImpl();
		
		try{
		Category category=caservice.findCategoryById(Integer.parseInt(id));
CateService cs=new CateServiceImpl();
		...
		if(category!=null) {
			...
			request.getRequestDispatcher("mng/updateCate.jsp").forward(request, response);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			category.setCreate_time(create_date);
